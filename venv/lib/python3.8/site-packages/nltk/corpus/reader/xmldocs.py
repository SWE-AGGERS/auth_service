...
class XMLCorpusView(StreamBackedCorpusView):
    """
    A corpus views that selects out specified elements from an XML
    ...
    Every XML corpus views has a "tag specification", indicating what
    XML elements should be included in the views; and each (non-nested)
    element that matches this specification corresponds to one item in
    the views.  Tag specifications are regular expressions over tag
    ...
    The views items are generated from the selected XML elements via
    ...
    def __init__(self, fileid, tagspec, elt_handler=None):
        """
        Create a new corpus views based on a specified XML file.
        ...
        """The tag specification for this corpus views."""
    ...
    def handle_elt(self, elt, context):
        ...
        the views.  Unless overridden by a subclass or by the
        ``elt_handler`` constructor argument, this method simply
        returns ``elt``.

        :return: The views value corresponding to ``elt``.

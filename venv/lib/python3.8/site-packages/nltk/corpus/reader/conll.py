...
class ConllCorpusReader(CorpusReader):
    ...
    @todo: Possibly add caching of the grid corpus views?  This would
        allow the same grid views to be used by different data access
        methods (eg words() and parsed_sents() could both share the
        same grid corpus views object).
    ...
    def _grids(self, fileids=None):
        ...
        # fileids), which would let us reuse the same corpus views for
